...
export type TeamMember = { userId: string; teamId: string, access: AccessType; name: string; uniformNumbers: number[]; positions : PlayerPositions[]};

export const useTeams = () => {
  const client = useSupabaseClient();

  const addMembersToTeam = (teamId: string, members: TeamMember[]) => {
    const mappedMembers = members.map(({ userId, access, name,  uniformNumbers , positions}) => ({
      team_id: teamId,
      name: name,
      user_id: userId,
      uniform_numbers: uniformNumbers,
      access: access,
      positions: positions,
    }));
